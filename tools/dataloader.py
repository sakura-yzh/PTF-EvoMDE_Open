...
from utils_newcrfs import DistributedSamplerNoEvenlyDivisible
...
class NewDataLoader(object):
    def __init__(self, args, mode):
        if mode == 'train':
            ...
            self.training_samples = DataLoadPreprocess(args, 'train', transform=preprocessing_transforms('train'))
            dataset_size = len(self.training_samples)
            ...
            train_subset = torch.utils.data.Subset(self.training_samples, train_indices)
            arch_subset = torch.utils.data.Subset(self.training_samples, arch_indices)

            if args.distributed:
                self.train_sampler = torch.utils.data.distributed.DistributedSampler(train_subset)
                self.arch_sampler = torch.utils.data.distributed.DistributedSampler(arch_subset)
            else:
                self.train_sampler = None
                self.arch_sampler = None

            self.train_data = DataLoader(train_subset, args.batch_size,
                                         shuffle=(self.train_sampler is None),
                                         num_workers=args.num_threads,
                                         pin_memory=True,
                                         sampler=self.train_sampler)

            self.arch_data = DataLoader(arch_subset, args.batch_size,
                                        shuffle=(self.arch_sampler is None),
                                        num_workers=args.num_threads,
                                        pin_memory=True,
                                        sampler=self.arch_sampler)
